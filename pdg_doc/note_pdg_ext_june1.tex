...
\noindent
	...
	%Denote the upper neighbor of the vertex $w_1$ in $G_k$ by $\mathcal{N}_{w_1}^{+,G_k} = \{ j \leq k \vert \sigma_{w_1j} \in G_k \}$. And denote the number of connected components in $G_k^{w_1+1}$ that contain at elements of $\mathcal{N}_{w_1}^{+,G_k}$ by $c_{w_1+1}^k$. Among these $c_{w_1+1}^k$ connected components, denote the one contained the vertex $k$ by $C_{w_1+1}^k(k)$. Let $\mathcal{V}_{kw_1}^{G_k} = \mathcal{N}_{w_1}^{+,G_k} \cap C_{w_1+1}^k(k)$ and $b_{w_1}^k = \vert \mathcal{V}_{kw_1}^{G_k} \vert$ be the set and the number of vertices in upper neighbors of $w_1$ that are connected to the vertex $k$.
	
	%For the point $(f(k),f(w_1))$ with the highest y-coordinate, after the Corollary 5.1, any pairs of vertices $(u_i,u_j) \in (\mathcal{U}_{kw_1}^{G_k})^2$ has a highest path from $u_i$ to $u_j$ whose minimum vertex equals to $w_1$. This means that a path from $u_1$ to $w_1$. Moreover, the Corollary 5.2 implies that a path from a vertex $u_i$ to $w_1$ shares no commom edges with a path from another vertex $u_j$ to $w_1$.
	
	%Each path from $u_i$ to $w_1$ contains a upper neighbor of $w_1$, and because $u_i$ is connected to $k$, thus  $\mathcal{U}_{kw_1}^{G_k} \subset C_{w_1+1}^{k}(k)$. Because the paths from $u_i$ to $w_1$ shares no edges between different $i =1,\ldots d^{w_1}_k$, the $d^{w_1}_k$ vertex sets $\{v_{i_1},\ldots v_{i_{p_i} } \} \subset \mathcal{V}_{kw_1}^{G_k}$ connected to $u_i$ for $i = 1,\ldots d_k^{w_1}$ form a partition of $\mathcal{V}_{kw_1}^{G_k}$.
